...
BEGIN TRANSACTION
	...
	IF EXISTS (SELECT TOP 1 1 FROM msdb.dbo.sysjobservers WHERE sysjobservers.job_id =
					(SELECT job_id FROM msdb.dbo.sysjobs WHERE name = N'sp_Cubo_Inicia_CxO'))
			BEGIN
	...
	-- Elimino el jobstep
	IF EXISTS (SELECT TOP 1 1 FROM msdb.dbo.sysjobsteps WHERE step_name  = N'sp_Cubo_Inicia_CxO')
	BEGIN
		DELETE FROM msdb.dbo.sysjobsteps WHERE step_name  = N'sp_Cubo_Inicia_CxO'
	END

	-- Elimino el job
	IF EXISTS (SELECT TOP 1 1 FROM msdb.dbo.sysjobs WHERE name = N'sp_Cubo_Inicia_CxO')
	BEGIN
		DELETE FROM msdb.dbo.sysjobs WHERE name = N'sp_Cubo_Inicia_CxO'
	END
	...
	EXEC @ReturnCode =  msdb.dbo.sp_add_job 
	  ...
	  @notify_level_page=0, 
	  @delete_level=0, 
	  @description=N'Ejecuta el SP [sp_Cubo_Inicia_CxO]', 
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep 
	  @job_id=@jobId,
	  @step_name=N'Step_sp_Cubo_Inicia_CxO', 
	  @step_id=1, 
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, 
	  ...
	  @active_start_time=30000, -- Tiempo en cualquier dia entre los parametros   @active_start_date y   @active_end_date en la que puede comenzar la ejecucion del JOB
