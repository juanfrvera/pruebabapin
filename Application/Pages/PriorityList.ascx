<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="PriorityList.ascx.cs" Inherits=" UI.Web.PriorityList" %>
...
<asp:GridView ID="Grid" runat="server"  Width="100%"
    ...
    >
    <Columns> 
<asp:BoundField DataField="IdPriority" HeaderText="Prioridad" SortExpression="IdPriority" HeaderStyle-Width="8%"/>
	<asp:TemplateField   HeaderText="Nombre"  SortExpression="Name" HeaderStyle-Width="63%" >            
            <ItemTemplate>
                <asp:Label ID="lblName" runat="server" Text='<%# Contract.DataHelper.CutString((string)Eval("Name"),25) %>' ToolTip='<%# Eval("Name") %>'  ></asp:Label>
            </ItemTemplate>
    </asp:TemplateField>
	<asp:TemplateField   HeaderText="Img"  SortExpression="Img" HeaderStyle-Width="20%" >            
            <ItemTemplate>
                <asp:Label ID="lblImg" runat="server" Text='<%# Contract.DataHelper.CutString((string)Eval("Img"),25) %>' ToolTip='<%# Eval("Img") %>'  ></asp:Label>
            </ItemTemplate>
    </asp:TemplateField>
