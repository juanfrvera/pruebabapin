<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ProyectoTipoList.ascx.cs" Inherits=" UI.Web.ProyectoTipoList" %>
...
<asp:BoundField DataField="Sigla" HeaderText="Sigla" SortExpression="Sigla" HeaderStyle-Width="8%"/>
    ...
    <asp:BoundField DataField="Tipo" HeaderText="T.Proyecto" SortExpression="Tipo" HeaderStyle-Width="20%"/>
	<asp:CheckBoxField DataField="Activo" HeaderText="Activo" SortExpression="Activo" HeaderStyle-Width="8%"/>
		<asp:TemplateField  ItemStyle-HorizontalAlign="Right" HeaderStyle-Width="9%">           
            <ItemTemplate>                
				...
				&nbsp;
				<asp:ImageButton ID ="btDelete" runat ="server" Visible="false" src="../Images/delete.jpg" ToolTip ='<%# Translate("Eliminar") %>'  CssClass='<%# CanDelete(Eval("ID"))?"":"ibDisable" %>' Enabled='<%# CanDelete(Eval("ID")) %>'   CommandName='<%# Command.DELETE %>' OnClientClick='<%#  "return confirm(\""+ConfirmDeleteMessage+"\")" %>' CommandArgument='<%#Eval("ID")%>' CausesValidation="false" />            
       	     </ItemTemplate>            
            <ItemStyle />
        </asp:TemplateField>
    </Columns>
