...
{
    public class ProyectoEtapaRealizadoData : EntityData<ProyectoEtapaRealizado, ProyectoEtapaRealizadoFilter, ProyectoEtapaRealizadoResult, int>
    {
        #region Singleton
        private static volatile ProyectoEtapaRealizadoData current;
        private static object syncRoot = new Object();

        //private ProyectoEtapaRealizadoData() {}
        public static ProyectoEtapaRealizadoData Current
        {
            get
            {
                if (current == null)
                {
                    ...
                    {
                        if (current == null)
                            current = new ProyectoEtapaRealizadoData();
                    }
                }
                return current;
            }
        ...
        #endregion
        public override string IdFieldName { get { return "IdProyectoEtapaRealizado"; } }

        public List<ProyectoEtapaRealizadoResult> GetEtapasRealizadas(ProyectoEtapaFilter filter)
        {
            List<ProyectoEtapaRealizadoResult> retval = new List<ProyectoEtapaRealizadoResult>();
            if (filter.IdProyecto > 0)
            {
                ...
                var pees = (from pee in this.Table
                            ...
                            select pee);
                var peeps = (from pp in this.Context.ProyectoEtapaRealizadoPeriodos
                             join pee in pees on pp.IdProyectoEtapaRealizado equals pee.IdProyectoEtapaRealizado
                             select pp);
                List<ClasificacionGasto> gastos = (from pp in this.Context.ProyectoEtapaRealizadoPeriodos
                                                   join pee in pees on pp.IdProyectoEtapaRealizado equals pee.IdProyectoEtapaRealizado
                                                   join g in this.Context.ClasificacionGastos on pee.IdClasificacionGasto equals g.IdClasificacionGasto
                                                   select g).Distinct().ToList();
                //List<ProyectoOrigenFinanciamientoResult> of = (from pp in this.Context.ProyectoEtapaRealizadoPeriodos
                //                                               join pee in pees on pp.IdProyectoEtapaRealizado equals pee.IdProyectoEtapaRealizado
                ...
                //                                               }).Distinct().ToList();
                List<FuenteFinanciamiento> ff = (from pp in this.Context.ProyectoEtapaRealizadoPeriodos
                                                 join pee in pees on pp.IdProyectoEtapaRealizado equals pee.IdProyectoEtapaRealizado
                ...
                foreach (ProyectoEtapaRealizado pee in pees.ToList())
                {
                    ProyectoEtapaRealizadoResult peer = new ProyectoEtapaRealizadoResult();
                    peer.IdProyectoEtapa = pee.IdProyectoEtapa;
                    peer.IdProyectoEtapaRealizado = pee.IdProyectoEtapaRealizado;
                    ...
                    //}
                    if (peer.IdClasificacionGasto > 0)
                    {
                    ...
                    if (peer.IdFuenteFinanciamiento > 0)
                    {
                    ...
                    foreach (ProyectoEtapaRealizadoPeriodo peep in peeps.Where(t => t.IdProyectoEtapaRealizado == pee.IdProyectoEtapaRealizado).ToList())
                    {
                        ProyectoEtapaRealizadoPeriodoResult peepr = new ProyectoEtapaRealizadoPeriodoResult();
                        peepr.IdProyectoEtapaRealizado = peep.IdProyectoEtapaRealizado;
                        peepr.IdProyectoEtapaRealizadoPeriodo = peep.IdProyectoEtapaRealizadoPeriodo;
                        peepr.Periodo = peep.Periodo;
                        peepr.Monto = peep.Monto;
                        peepr.Cotizacion = peep.Cotizacion;
                        peepr.MontoCalculado = peep.MontoCalculado;
                        peepr.Fecha = peep.Fecha;
        ...
        #endregion		
		#region Get
		public override int GetId(nc.ProyectoEtapaRealizado entity)
		{			
			return entity.IdProyectoEtapaRealizado;
		}		
		public override ProyectoEtapaRealizado GetByEntity(ProyectoEtapaRealizado entity)
        {
            return this.GetById(entity.IdProyectoEtapaRealizado);
        }
        public override ProyectoEtapaRealizado GetById(int id)
        {
            return (from o in this.Table where o.IdProyectoEtapaRealizado == id select o).FirstOrDefault();
        }
		#endregion
		#region Query
		protected override IQueryable<ProyectoEtapaRealizado> Query(ProyectoEtapaRealizadoFilter filter)
        {
			return (from o in this.Table
                      where (filter.IdProyectoEtapaRealizado == null || filter.IdProyectoEtapaRealizado == 0 || o.IdProyectoEtapaRealizado==filter.IdProyectoEtapaRealizado)
        ...
        protected override IQueryable<ProyectoEtapaRealizadoResult> QueryResult(ProyectoEtapaRealizadoFilter filter)
        {
		  return (from o in Query(filter)					
				   ...
				   join _t3  in this.Context.Monedas on o.IdMoneda equals _t3.IdMoneda into tt3 from t3 in tt3.DefaultIfEmpty()
				    join t4  in this.Context.ProyectoEtapas on o.IdProyectoEtapa equals t4.IdProyectoEtapa   
				   select new ProyectoEtapaRealizadoResult(){
					...
					,ClasificacionGasto_Codigo= t1.Codigo	
                        ...
                        //,Moneda_Sigla= t3!=null?(string)t3.Sigla:null	
                        //,Moneda_Nombre= t3!=null?(string)t3.Nombre:null	
                        //,Moneda_Activo= t3!=null?(bool?)t3.Activo:null	
                        //,Moneda_Base= t3!=null?(bool?)t3.Base:null	
		...
		#endregion
		#region Copy
		public override nc.ProyectoEtapaRealizado Copy(nc.ProyectoEtapaRealizado entity)
        {           
            nc.ProyectoEtapaRealizado _new = new nc.ProyectoEtapaRealizado();
		...
		public override int CopyAndSave(ProyectoEtapaRealizado entity,string renameFormat)
        {
            ProyectoEtapaRealizado  newEntity = Copy(entity);
            Add(newEntity);
			return GetId(newEntity);
        }
		#endregion
		#region Set
		public override void SetId(ProyectoEtapaRealizado entity, int id)
        {            
            entity.IdProyectoEtapaRealizado = id;            
        }
		public override void Set(ProyectoEtapaRealizado source,ProyectoEtapaRealizado target,bool hadSetId)
		{		   
		if(hadSetId)target.IdProyectoEtapaRealizado= source.IdProyectoEtapaRealizado ;
		...
		public override void Set(ProyectoEtapaRealizadoResult source,ProyectoEtapaRealizado target,bool hadSetId)
		{		   
		if(hadSetId)target.IdProyectoEtapaRealizado= source.IdProyectoEtapaRealizado ;
		 target.IdProyectoEtapa= source.IdProyectoEtapa ;
		 target.IdClasificacionGasto= source.IdClasificacionGasto ;
		...
		public override void Set(ProyectoEtapaRealizado source,ProyectoEtapaRealizadoResult target,bool hadSetId)
		{		   
		if(hadSetId)target.IdProyectoEtapaRealizado= source.IdProyectoEtapaRealizado ;
		 target.IdProyectoEtapa= source.IdProyectoEtapa ;
		 target.IdClasificacionGasto= source.IdClasificacionGasto ;
		...
		public override void Set(ProyectoEtapaRealizadoResult source,ProyectoEtapaRealizadoResult target,bool hadSetId)
		{		   
		if(hadSetId)target.IdProyectoEtapaRealizado= source.IdProyectoEtapaRealizado ;
		 target.IdProyectoEtapa= source.IdProyectoEtapa ;
		 target.IdClasificacionGasto= source.IdClasificacionGasto ;
		...
		#endregion			
		#region Equals
		public override bool Equals(ProyectoEtapaRealizado source,ProyectoEtapaRealizado target)
        {
         ...
         if(!source.IdProyectoEtapaRealizado.Equals(target.IdProyectoEtapaRealizado))return false;
		  ...
		  if((source.IdMoneda == null)?(target.IdMoneda.HasValue && target.IdMoneda.Value > 0):!source.IdMoneda.Equals(target.IdMoneda))return false;
						 
		  return true;
        }
		public override bool Equals(ProyectoEtapaRealizadoResult source,ProyectoEtapaRealizadoResult target)
        {
		   if(source == null && target == null)return true;
		   if(source == null )return false;
		   if(target == null)return false;
