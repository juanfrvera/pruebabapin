	...
	BEGIN TRAN
		...
		IF NOT EXISTS(SELECT 1 FROM sys.tables WHERE Name = 'ProyectoDictamenFile' AND OBJECT_ID = OBJECT_ID('ProyectoDictamenFile'))
			BEGIN
				...
				--Verifico si la instruccion Fecth se ejecuto correctamente
...
-- Creo la columna IdProyectoOrigenFinanciamiento para la tabla PrestamoProducto

IF NOT EXISTS(SELECT TOP 1 1
	FROM INFORMATION_SCHEMA.COLUMNS
	WHERE COLUMN_NAME = 'IdProyectoOrigenFinanciamiento' AND TABLE_NAME = 'PrestamoProducto')
	BEGIN
		ALTER TABLE PrestamoProducto ADD IdProyectoOrigenFinanciamiento INT NOT NULL DEFAULT 0
		
		PRINT 'Columna IdProyectoOrigenFinanciamiento creada correctamente en la tabla PrestamoProducto'
	END

GO

-- Creo la tabla MessageConfiguration

IF NOT EXISTS(SELECT 1 FROM sys.tables WHERE Name = 'MessageConfiguration' AND OBJECT_ID = OBJECT_ID('MessageConfiguration'))
BEGIN
	CREATE TABLE dbo.MessageConfiguration  
	   (IdMessageConfiguration INT PRIMARY KEY IDENTITY (1,1) NOT NULL,  
		IdMessage INT NOT NULL,  
		Page VARCHAR(150) NOT NULL,
		OperationType INT NOT NULL
		)

	ALTER TABLE [dbo].[MessageConfiguration]  WITH CHECK ADD  CONSTRAINT [FK_MessageConfiguration_Message] FOREIGN KEY([IdMessage])
	REFERENCES [dbo].[Message] ([IdMessage])
	
	PRINT 'Tabla MessageConfiguration creada correctamente'
END
GO

-- Modificar el nombre y codigo de los registros POSTULADO Y CONFORMADO, se deberan invertir

	UPDATE EstadoDeDesicion SET
		CODIGO = 'Postulado',
		NOMBRE = 'Postulado'
	WHERE IdEstadoDeDesicion = 2

	UPDATE EstadoDeDesicion SET
		CODIGO = 'Conformado',
		NOMBRE = 'Conformado'
	WHERE IdEstadoDeDesicion = 1

	UPDATE EstadoDeDesicion SET
		CODIGO = 'Observado',
		NOMBRE = 'Observado'
	WHERE NOMBRE = 'Rechazado' AND Codigo = 'Rechazado'

GO



-- Modifico la columna Text.Code (antes era varchar 50, ahora 100)

IF EXISTS ( SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[dbo].[Text]') AND name = 'Code' )
BEGIN
	
	ALTER TABLE [dbo].[Text]
	ALTER COLUMN Code varchar(100)
	
	PRINT 'Longuitud de Text.Code modificado correctamente'
END
GO
...
-- Actulizo el ultimo plan de cada proyecto (el MAX_ID).

update	Proyecto	
set		IdProyectoPlan = (select MAX(pp.idProyectoPlan) from ProyectoPlan pp where pp.IdProyecto = Proyecto.IdProyecto)
